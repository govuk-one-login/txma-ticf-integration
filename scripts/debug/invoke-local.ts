 ...
 * Usage: ts-node scripts/debug/invoke-local.ts <lambdaName>
 * e.g.   ts-node scripts/debug/invoke-local.ts initiateDataRequest
 */
...
if (!process.env.AWS_PROFILE) {
  process.env.AWS_PROFILE = 'audit-dev'
}
...
const context: Context = {
  ...
  invokedFunctionArn: `arn:aws:lambda:eu-west-2:123456789012:function:${lambdaName}`,
...
const { handler } = require(
  join(rootDir, 'src/lambdas', lambdaName, 'handler')
) as {
